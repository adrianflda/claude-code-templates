...
argument-hint: "[optional: 'tdd' for TDD mode or specific focus area]"
allowed-tools: Bash(git:*), Bash(date:*), Read, Write
---
...
### Step 1: Detect Active Feature and Validate
...
- **Required**: `spec.md` (user stories + priorities)
- **Optional**: `data-model.md` (entities), `contracts/` (interfaces), `research.md` (decisions)

If `spec.md` is missing, STOP: "spec.md not found. Run `/sdd-specify` first to create the feature specification."

If `plan.md` is missing, STOP: "plan.md not found. Run `/sdd-plan` first to generate the technical plan."
...
- Testing framework (if TDD requested or required by constitution)
...
- Tests flagged as TDD ONLY if explicitly requested in spec, constitution, or `$ARGUMENTS`
...
- Set Tasks to ✅ with task count and phase count
...
Next steps:
  ...
  2. Run /sdd-analyze to verify cross-artifact consistency (recommended)
